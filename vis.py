...
def main(args=None):
    ...
    parser.add_argument('--cat_id',default = None)
    ...
    cat_name = random.randint(0,len(cat_names)-1)
   
    catids = coco_true.getCatIds(catNms=cat_name)
    imgids = coco_true.getImgIds(catIds=catids)

    # You can set to a certain id to visualize same images for comparison
    parser.cat_id  = 4414
    iimg = coco_true.loadImgs(imgids)
    for i in iimg:
        if i['id'] ==  parser.cat_id:
            img = i
